...
<div class="row configuracion-input shadow border border-light py-5 px-3 mt-3 {{ ($subcampo)?'subcampo':'' }}">
    ...
    <h5 class="text-dark">Campo de fecha</h5>
    <input type="hidden" name="{{ $subcampo ? 'input' : 'input[]' }}" value="date">
   {{-- ---> datos básicos --}}
    ...
    <div class="col-sm-6 mt-3 mb-5">
        <label for="" class="form-label">Formato de fecha</label>
        @if ($subcampo)
             <input type="text" placeholder="fecha_inicio" class="formato-fecha form-control" name="formato_fecha" value="Y-m-d">
        @else
             <input type="text" placeholder="fecha_inicio" class="formato-fecha form-control" name="{{ isset($input)?'inputs['.$i.'][formato_fecha]':'formato_fecha[]' }}" value="Y-m-d">
        @endif
