...
<div class="row configuracion-input shadow border border-light py-5 px-3 mt-3 {{ ($subcampo)?'subcampo':'' }}">
    ...
    <h5 class="text-dark">Campo de tiempo</h5>
    <input type="hidden" name="{{ $subcampo ? 'input' : 'input[]' }}" value="time">
   {{-- ---> datos básicos --}}
    ...
    <div class="col-sm-6 mt-3 mb-5">
        ...
        <input type="text" placeholder="hora_inicio" class="form-control" name="{{ $subcampo ? 'name' : 'name[]' }}" value="{{ $input["name"] ?? '' }}">
    </div>

    <div class="col-sm-6 mt-3 mb-5">
        <label for="" class="form-label">Formato de tiempo</label>
        @if ($subcampo)
