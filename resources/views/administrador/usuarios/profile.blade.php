          ...
          <div class="card-body p-4">
             ...
             <div class="position-relative mb-5 background-profile rounded-3">
              ...
              <div class="profile-avatar position-absolute top-100 start-50 translate-middle">
                <img src="{{ ($avatar->avatar==null)?"dashboard_resources/assets/images/avatars/default.jpg":Storage::url("avatares/".$avatar->avatar); }}" class="img-fluid rounded-circle p-1 bg-grd-danger shadow" width="170" height="170" alt="">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li><a class="dropdown-item" href="javascript:;">Something else here</a></li>
									...
									<div class="col-md-6">
										...
										<input type="text" class="form-control" id="input1" placeholder="Nombre" value="{{ Auth::user()->name }}">
									</div>
									<div class="col-md-6">
										<label for="input2" class="form-label">Apellidos</label>
										<input type="text" class="form-control" id="input2" placeholder="Apellidos" value="{{ Auth::user()->lastname }}">
									</div>
									<div class="col-md-12">
										<label for="input3" class="form-label">Teléfono</label>
										<input type="text" class="form-control" id="input3" placeholder="Phone" value="{{ Auth::user()->telephone }}">
									</div>
									<div class="col-md-12">
										<label for="input4" class="form-label">Correo</label>
										<input type="email" class="form-control" id="input4" value="{{ Auth::user()->email }}">
									</div>
                        ...
                        <select id="input21" class="form-select  pt-3" name="estado">
                            @foreach(Auth::user()::$estados as $estado)
                                <option {{ (Auth::user()->estado==$estado)? "selected":"" }} value="{{ $estado }}">{{ $estado }}</option>
                            @endforeach
                        </select>
                </div>
                ...
                <div class="d-flex align-items-start justify-content-between mb-3">
                 ...
                 <div class="full-info">
                  <div class="info-list d-flex flex-column gap-3">
                    <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">account_circle</span><p class="mb-0">Nombre completo: {{ Auth::user()->name." ".Auth::user()->lastname }}</p></div>
                    <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">done</span><p class="mb-0">Estatus: {{ Auth::user()->estado }} </p></div>
                    <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">code</span><p class="mb-0">Rol: 
                    @foreach ($roles as $role)
                      <button type="button" class="btn btn-sm btn-light rounded-5 px-4">{{ $role->name }}</button>
                    @endforeach</p></div>
                    ...
                    <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">call</span><p class="mb-0">Phone: {{ Auth::user()->telephone}}</p></div>
                  </div>
                </div>
              </div>
            </div>
