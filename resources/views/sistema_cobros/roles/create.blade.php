...
<div class="card-header pt-3">
    <h5>Crear una inscripciones</h5>
</div>
<div class="card-body">
    <form class="row" method="post" action="{{ route('inscripciones.store') }}" enctype="multipart/form-data">
        @csrf
        <x-lista-mensajes/>
        <div class="row">
            ...
            <div class="col-12">
                @php
                    $opciones = array(["id"=>"Escolarizado","option"=>"Escolarizado"],["id"=>"Semi-Escolarizado","option"=>"Semi-Escolarizado"])
                @endphp
                <x-dropdown-formulario label="Modalidad" id="activo" :options="$opciones" value-key="id" option-key="option" simpleArray="true" name="modalidad" />
            </div>
