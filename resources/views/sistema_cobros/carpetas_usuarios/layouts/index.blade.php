...
<head>
    @include('general.partials.head')
      ...
      <link href="{{  asset("dashboard_resources/assets/plugins/Drag-And-Drop/dist/imageuploadify.min.css");}}" rel="stylesheet">
</head>
<body>
	...
	<script src="{{  asset("dashboard_resources/assets/plugins/Drag-And-Drop/dist/imageuploadify.min.js");}}"></script>
    <script>
        ...
        $('.cursor-pointer').dblclick(function() {
            var baseUrl = '{{ route("ver_contenido", ["id_directorio" => $id_directorio]) }}';
            var direccion = $(this).attr("data-pointer");
            window.location.href = baseUrl + '/' + encodeURIComponent(direccion);
